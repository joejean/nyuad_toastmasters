...
from forms import SignupForm, AdminForm

from models import Member, Admin
from config import MAIL_DEFAULT_SENDER, ADMINS
...
def landing():
    ...
    if form.validate_on_submit():
        try:
            ...
            member = models.Member(net_id= form.netID.data, name=form.name.data)
            ...
            db.session.rollback()
            flash("Sorry you can't sign up twice", "error")
            return redirect(url_for('landing'))
    return render_template('signup.html', title ="Welcome", form=form)

@app.route('/123Admin45')
def admin():
    
    return render_template('admin.html', members=g.members)
